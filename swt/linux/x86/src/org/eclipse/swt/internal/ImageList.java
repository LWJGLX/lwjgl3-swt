...
public static int /*long*/ createPixbuf(Image image) {
	...
	int /*long*/ pixbuf = GDK.gdk_pixbuf_new (GDK.GDK_COLORSPACE_RGB, hasAlpha, 8, width, height);
...
void set (int index, Image image) {
	...
	if (w != width || h != height) {
		int /*long*/ scaledPixbuf = GDK.gdk_pixbuf_scale_simple(pixbuf, width, height, GDK.GDK_INTERP_BILINEAR);
