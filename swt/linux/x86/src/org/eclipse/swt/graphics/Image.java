...
void initNative(String filename) {
	try {
		...
		int /*long*/ pixbuf = GDK.gdk_pixbuf_new_from_file(buffer, null);
		if (pixbuf != 0) {
			try {
...
void createSurface() {
	...
	if (mask != 0 || alpha != -1 || alphaData != null) {
		...
		if (mask != 0 && GDK.gdk_drawable_get_depth(mask) == 1) {
			int /*long*/ maskPixbuf = GDK.gdk_pixbuf_new(GDK.GDK_COLORSPACE_RGB, false, 8, width, height);
			...
			int /*long*/ maskPixels = GDK.gdk_pixbuf_get_pixels(maskPixbuf);
		...
		int /*long*/ xDrawable = GDK.GDK_PIXMAP_XID(pixmap);
...
public int /*long*/ internal_new_GC (GCData data) {
	...
	int /*long*/ gc = Cairo.cairo_create(surface);
	if (data != null) {
