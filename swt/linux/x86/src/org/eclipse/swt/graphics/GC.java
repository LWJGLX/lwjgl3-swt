...
void cairoClipRegion (int /*long*/ cairo) {
	...
	int /*long*/ regionHandle = data.regionSet;
...
int /*long*/ convertRgn(int /*long*/ rgn, double[] matrix) {
	int /*long*/ newRgn = GDK.gdk_region_new();
	...
	for (int i=0; i<nRects[0]; i++) {
		...
		int /*long*/ polyRgn = Region.gdk_region_polygon(pointArray, pointArray.length / 2, GDK.GDK_EVEN_ODD_RULE);
...
void copyAreaInPixels(int srcX, int srcY, int width, int height, int destX, int destY, boolean paint) {
	...
	if (data.image != null) {
		...
		if (paint) {
			...
			int /*long*/ copyRegion = GDK.gdk_region_rectangle (srcRect);
			GDK.gdk_region_intersect(copyRegion, visibleRegion);
			int /*long*/ invalidateRegion = GDK.gdk_region_rectangle (srcRect);
...
void createLayout() {
	int /*long*/ context = GDK.gdk_pango_context_get();
...
void drawFocusInPixels(int x, int y, int width, int height) {
	...
	checkGC(FOREGROUND);
	if (GTK.GTK3) {
		int /*long*/  context = GTK.gtk_widget_get_style_context(data.device.shellHandle);
...
Rectangle getClippingInPixels() {
	...
	if (clipRgn != 0 || damageRgn != 0 || cairo != 0) {
		int /*long*/ rgn = GDK.gdk_region_new();
		GdkRectangle rect = new GdkRectangle();
		rect.width = width;
		rect.height = height;
...
void setCairoClip(int /*long*/ damageRgn, int /*long*/ clipRgn) {
	...
	if (clipRgn != 0) {
		int /*long*/ clipRgnCopy = GDK.gdk_region_new();
 ...
 */
private void limitClipping(int /*long*/ gcClipping) {
...
void setClippingInPixels(int x, int y, int width, int height) {
	...
	rect.width = width;
	rect.height = height;
	int /*long*/ clipRgn = GDK.gdk_region_new();
