...
static int /*long*/ gdk_region_polygon(int[] pointArray, int npoints, int fill_rule) {
	...
	int /*long*/ polyRgn = GDK.gdk_cairo_region_create_from_surface(surface);
...
void addInPixels (int[] pointArray) {
	...
	*/
	...
	int /*long*/ polyRgn = gdk_region_polygon(pointArray, pointArray.length / 2, GDK.GDK_EVEN_ODD_RULE);
...
void intersectInPixels(int x, int y, int width, int height) {
	...
	gdkRect.width = width;
	gdkRect.height = height;
	int /*long*/ rectRgn = GDK.gdk_region_rectangle(gdkRect);
