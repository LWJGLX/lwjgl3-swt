...
public abstract class Device implements Drawable {
	...
	/* Settings callbacks */
	int /*long*/ gsettingsProc;
	Callback gsettingsCallback;
	boolean isConnected = false;
	int /*long*/ displaySettings; //gsettings Dictionary
...
Point getScreenDPI () {
	...
	if (GTK.GTK3) {
		if (GTK.GTK_VERSION >= OS.VERSION(3, 22, 0)) {
			...
			int /*long*/ pMonitor = GDK.gdk_display_get_primary_monitor(display);
			if (pMonitor == 0) {
...
protected void init () {
	...
	if (GTK.GTK_VERSION >= OS.VERSION(3, 22, 0)) {
		int /*long*/ surface = GDK.gdk_window_create_similar_surface(GDK.gdk_get_default_root_window(), Cairo.CAIRO_CONTENT_COLOR, 10, 10);
	...
	int /*long*/ [] defaultFontArray = new int /*long*/ [1];
