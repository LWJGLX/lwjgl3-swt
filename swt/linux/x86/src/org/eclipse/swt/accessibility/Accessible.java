...
public class Accessible {
	...
	Accessible (Control control) {
		...
		if (GTK.GTK3) {
			int /*long*/ type = OS.G_OBJECT_TYPE (getControlHandle());
	...
	AccessibleObject getAccessibleObject () {
		if (accessibleObject == null) {
			if (GTK.GTK3) {
				int /*long*/ widget = this.getControlHandle();
				int /*long*/ type = OS.G_OBJECT_TYPE (widget);
				if (parent == null) {
	...
	int /*long*/ getControlHandle () {
		...
		if (control instanceof Label) {
			int /*long*/ list = GTK.gtk_container_get_children (result);
			if (list != 0) {
				int /*long*/ temp = list;
				while (temp != 0) {
