...
public PrinterData open() {
	...
	int /*long*/ settings = GTK.gtk_print_settings_new();
	int /*long*/ page_setup = GTK.gtk_page_setup_new();
	...
	*/
	...
	if (response == GTK.GTK_RESPONSE_OK) {
		int /*long*/ printer = GTK.gtk_print_unix_dialog_get_selected_printer(handle);
		if (printer != 0) {
			...
			switch (print_pages) {
				...
				case GTK.GTK_PRINT_PAGES_RANGES:
					...
					int /*long*/ page_ranges = GTK.gtk_print_settings_get_page_ranges(settings, num_ranges);
					int [] pageRange = new int[2];
			...
			if (data.printToFile) {
				int /*long*/ address = GTK.gtk_print_settings_get(settings, GTK.GTK_PRINT_SETTINGS_OUTPUT_URI);
			...
			int /*long*/ paper_size = GTK.gtk_page_setup_get_paper_size(page_setup); //$NON-NLS-1$
