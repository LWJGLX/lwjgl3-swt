...
public void append (String string) {
	...
	if ((style & SWT.SINGLE) != 0) {
		...
		int /*long*/ mark = GTK.gtk_text_buffer_get_insert (bufferHandle);
...
void applySegments () {
	...
	if ((style & SWT.SINGLE) != 0) {
		...
		for (int i = 0; i < nSegments; i++) {
			...
			int /*long*/ ptr = GTK.gtk_text_buffer_get_text (bufferHandle, startIter, endIter, true);
...
public void clearSelection () {
	...
	if ((style & SWT.SINGLE) != 0) {
		...
		byte [] position = new byte [ITER_SIZEOF];
		int /*long*/ insertMark = GTK.gtk_text_buffer_get_insert (bufferHandle);
...
Point computeSizeInPixels (int wHint, int hHint, boolean changed) {
	...
	if ((style & SWT.SINGLE) != 0) {
		...
		int /*long*/ text = GTK.gtk_text_buffer_get_text (bufferHandle, start, end, true);
		int /*long*/ layout = GTK.gtk_widget_create_pango_layout (handle, text);
		OS.g_free (text);
	...
	if ((style & SWT.SINGLE) != 0 && message.length () > 0) {
		...
		int /*long*/ layout = GTK.gtk_widget_create_pango_layout (handle, buffer);
...
public void copy () {
	...
	if ((style & SWT.SINGLE) != 0) {
		...
		int /*long*/ clipboard = GTK.gtk_clipboard_get (GDK.GDK_NONE);
		clearSegments (true);
...
public int getCaretPosition () {
	...
	if ((style & SWT.SINGLE) != 0)  {
		...
		int /*long*/ ptr = GTK.gtk_text_buffer_get_text (bufferHandle, zero, position, true);
...
/*public*/ int getPosition (Point point) {
	...
	if ((style & SWT.SINGLE) != 0) {
		...
		byte [] zero = new byte [ITER_SIZEOF];
		GTK.gtk_text_buffer_get_iter_at_offset(bufferHandle, zero, 0);
		int /*long*/ ptr = GTK.gtk_text_buffer_get_text (bufferHandle, zero, p, true);
...
public Point getSelection () {
	...
	if ((style & SWT.SINGLE) != 0) {
		...
		byte [] zero = new byte [ITER_SIZEOF];
		GTK.gtk_text_buffer_get_iter_at_offset(bufferHandle, zero, 0);
		int /*long*/ ptr = GTK.gtk_text_buffer_get_text (bufferHandle, zero, endIter, true);
...
public int getTopIndex () {
	 ...
	 */
	if (GTK.GTK3) {
		int /*long*/ vAdjustment = GTK.gtk_scrollable_get_vadjustment (handle);
...
void drawMessage (int /*long*/ cr) {
	...
	if ((style & SWT.SINGLE) != 0 && message.length () > 0) {
		...
		if (!GTK.gtk_widget_has_focus (handle) && C.strlen (str) == 0) {
			...
			int /*long*/ context = GTK.gtk_widget_get_pango_context (handle);
			...
			if (GTK.GTK3) {
				int /*long*/ styleContext = GTK.gtk_widget_get_style_context (handle);
				GdkRGBA textRGBA;
				...
				int /*long*/ style = GTK.gtk_widget_get_style (handle);
...
public void showSelection () {
	checkWidget ();
	if ((style & SWT.SINGLE) != 0) return;
	int /*long*/ mark = GTK.gtk_text_buffer_get_selection_bound (bufferHandle);
	GTK.gtk_text_view_scroll_to_mark (handle, mark, 0, true, 0, 0);
