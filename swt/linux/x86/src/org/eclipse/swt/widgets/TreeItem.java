...
void clear () {
	...
	if (cached || (parent.style & SWT.VIRTUAL) == 0) {
		...
		for (int i=Tree.FOREGROUND_COLUMN; i<columnCount; i++) {
			GTK.gtk_tree_store_set (parent.modelHandle, handle, i, (int /*long*/)0, -1);
		}
	}
	cached = false;
	font = null;
...
Rectangle getBoundsInPixels (int index) {
	...
	int /*long*/ path = GTK.gtk_tree_model_get_path (parent.modelHandle, handle);
...
Rectangle getBoundsInPixels () {
	...
	if (!parent.checkData (this)) error (SWT.ERROR_WIDGET_DISPOSED);
	int /*long*/ parentHandle = parent.handle;
	int /*long*/ column = GTK.gtk_tree_view_get_column (parentHandle, 0);
	if (column == 0) return new Rectangle (0, 0, 0, 0);
...
public int indexOf (TreeItem item) {
	...
	int /*long*/ currentPath = GTK.gtk_tree_model_get_path (parent.modelHandle, handle);
	int /*long*/ parentPath = GTK.gtk_tree_model_get_path (parent.modelHandle, item.handle);
	...
	int /*long*/ path = GTK.gtk_tree_model_get_path (parent.modelHandle, item.handle);
	if (depth > 1) {
...
public void removeAll () {
	...
	int /*long*/ selection = GTK.gtk_tree_view_get_selection (parent.handle);
	int [] value = new int [1];
	...
	if (GTK.GTK3) {
		if (DPIUtil.useCairoAutoScale()) {
			...
			if ((!parent.ownerDraw) && (image != null) && (DPIUtil.getDeviceZoom() != 100)) {
				...
				int /*long*/ scaledPixbuf = GDK.gdk_pixbuf_scale_simple(pixbuf, imgSize.width, imgSize.height, GDK.GDK_INTERP_BILINEAR);
				if (scaledPixbuf !=0) {
					pixbuf = scaledPixbuf;
				}
			}
		...
		int /*long*/parentHandle = parent.handle;
		int /*long*/ column = GTK.gtk_tree_view_get_column (parentHandle, index);
		int /*long*/ pixbufRenderer = parent.getPixbufRenderer (column);
	...
	if ((parent.style & SWT.VIRTUAL) != 0 && parent.currentItem == null) {
		if (image != null) {
			...
			if (w[0] < iWidth) {
				 ...
				 */
				if (!GTK.GTK3) {
					int /*long*/ style = GTK.gtk_widget_get_modifier_style (parentHandle);
