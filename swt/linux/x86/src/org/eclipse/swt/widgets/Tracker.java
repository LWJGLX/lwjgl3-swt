...
void drawRectangles (Rectangle [] rects) {
	int /*long*/ window = GDK.gdk_get_default_root_window();
	if (parent != null) {
	...
	if (GTK.GTK3) {
		...
		int /*long*/ region = GDK.gdk_region_new ();
		...
		int /*long*/ overlayWindow = GTK.gtk_widget_get_window (overlay);
...
private void setTrackerBackground(boolean opaque) {
	...
	if (GTK.GTK_VERSION < OS.VERSION (3, 14, 0)) {
		...
		int /*long*/ context = GTK.gtk_widget_get_style_context (overlay);
		if (provider == 0) {
...
boolean processEvent (int /*long*/ eventPtr) {
	...
	int /*long*/ widget = GTK.gtk_get_event_widget (eventPtr);
