...
String computeResultChooserDialog () {
	...
	if ((style & SWT.MULTI) != 0) {
		...
		if (uriMode) {
			...
			int /*long*/ path = GTK.gtk_file_chooser_get_filename (handle);
			if (path != 0) {
	...
	filterIndex = -1;
	int /*long*/ filter = GTK.gtk_file_chooser_get_filter (handle);
	if (filter != 0) {
		int /*long*/ filterNamePtr = GTK.gtk_file_filter_get_name (filter);
		if (filterNamePtr != 0) {
...
String openChooserDialog () {
	...
	int /*long*/ pixbufs = GTK.gtk_window_get_icon_list (shellHandle);
	if (pixbufs != 0) {
...
void presetChooserDialog () {
	...
	for (int i = 0; i < filterExtensions.length; i++) {
		if (filterExtensions [i] != null) {
			int /*long*/ filter = GTK.gtk_file_filter_new ();
