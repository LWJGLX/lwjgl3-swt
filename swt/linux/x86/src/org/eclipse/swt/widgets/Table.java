...
int /*long*/ cellDataProc (int /*long*/ tree_column, int /*long*/ cell, int /*long*/ tree_model, int /*long*/ iter, int /*long*/ data) {
	...
	int /*long*/ path = GTK.gtk_tree_model_get_path (tree_model, iter);
	int [] index = new int [1];
...
void createColumn (TableColumn column, int index) {
	...
	if (columnCount != 0) {
		...
		if (modelIndex == modelLength) {
			...
			int /*long*/ newModel = GTK.gtk_list_store_newv (types.length, types);
	...
	if (columnCount == 0) {
		...
		for (int i=0; i<itemCount; i++) {
			...
			if (item != null) {
				...
				GTK.gtk_list_store_set (modelHandle, iter, modelIndex + CELL_PIXBUF, (int /*long*/)0, -1);
				GTK.gtk_list_store_set (modelHandle, iter, modelIndex + CELL_TEXT, (int /*long*/)0, -1);
				GTK.gtk_list_store_set (modelHandle, iter, modelIndex + CELL_FOREGROUND, (int /*long*/)0, -1);
				GTK.gtk_list_store_set (modelHandle, iter, modelIndex + CELL_BACKGROUND, (int /*long*/)0, -1);
				GTK.gtk_list_store_set (modelHandle, iter, modelIndex + CELL_FONT, (int /*long*/)0, -1);
...
TableItem getFocusItem () {
	...
	TableItem item = null;
	int /*long*/ indices = GTK.gtk_tree_path_get_indices (path [0]);
	if (indices != 0) {
		int [] index = new int []{-1};
...
public TableItem [] getSelection () {
	...
	if (list != 0) {
		...
		for (int i=0; i<count; i++) {
			...
			int /*long*/ indices = GTK.gtk_tree_path_get_indices (data);
			if (indices != 0) {
				int [] index = new int [1];
				C.memmove (index, indices, 4);
...
public boolean isSelected (int index) {
	...
	int /*long*/ path = GTK.gtk_tree_path_new_from_string (buffer);
...
public void setTopIndex (int index) {
	...
	topIndex = index;
	int /*long*/ path = GTK.gtk_tree_model_get_path (modelHandle, _getItem (index).handle);
