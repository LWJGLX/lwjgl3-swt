...
public class Composite extends Scrollable {
	 ...
	 */
	Map<Control, int /*long*/ []> fixClipMap = new HashMap<> ();
...
void createHandle (int index, boolean fixed, boolean scrolled) {
	if (scrolled) {
		...
		int /*long*/ vadj = GTK.gtk_adjustment_new (0, 0, 100, 1, 10, 10);
		if (vadj == 0) error (SWT.ERROR_NO_HANDLES);
		int /*long*/ hadj = GTK.gtk_adjustment_new (0, 0, 100, 1, 10, 10);
	...
	if (fixClipHandle == 0 || fixClipMap.isEmpty()) {
		...
		for (Control child : children) {
			if (fixClipMap.containsKey(child)) {
				int /*long*/ [] childHandles = fixClipMap.get(child);
				for (int /*long*/ widget : childHandles) {
		...
		if (control.backgroundImage != null) {
			...
			if (surface == 0) {
				...
				if (OS.isX11()) {
					...
					int /*long*/ xDrawable = GDK.GDK_PIXMAP_XID (drawable);
...
void fixZOrder () {
	...
	int /*long*/ windowList = GDK.gdk_window_get_children (parentWindow);
	if (windowList != 0) {
...
int /*long*/ gtk_expose_event (int /*long*/ widget, int /*long*/ eventPtr) {
	...
	for (int i=0; i<n_rectangles[0]; i++) {
		...
		int /*long*/ damageRgn = GDK.gdk_region_new ();
...
void connectFixedHandleDraw () {
	int /*long*/ paintHandle = fixedHandle;
 ...
 */
void propagateDraw (int /*long*/ container, int /*long*/ cairo) {
	if (container == fixedHandle && GTK.GTK3) {
		int /*long*/ list = GTK.gtk_container_get_children (container);
		int /*long*/ temp = list;
		while (temp != 0) {
			int /*long*/ child = OS.g_list_data (temp);
			if (child != 0) {
