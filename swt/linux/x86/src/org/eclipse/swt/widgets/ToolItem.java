...
void createHandle (int index) {
	...
	switch (style & bits) {
		...
		case SWT.DROP_DOWN:
			 ...
			 */
			int /*long*/ child = GTK.gtk_bin_get_child (handle);
			int /*long*/ list = GTK.gtk_container_get_children (child);
	...
	if (image != null) {
		...
		if (imageList != null) {
			...
			if (index != -1) {
				...
				if (settings != 0) {
					if (!GTK.GTK3) {
						int /*long*/ property = OS.g_object_class_find_property(OS.G_OBJECT_GET_CLASS(settings), GTK.gtk_menu_images);
				...
				if (GTK.GTK3) {
					...
					int /*long*/ boxHandle = gtk_box_new (GTK.GTK_ORIENTATION_HORIZONTAL, false, 6);
					if (boxHandle == 0) error (SWT.ERROR_NO_HANDLES);

					int /*long*/ menuLabel = GTK.gtk_accel_label_new (label);
					...
					int /*long*/ menuImage = GTK.gtk_image_new_from_pixbuf (pixbuf);
...
void hookEvents () {
	...
	if ((style & SWT.DROP_DOWN) != 0) {
		int /*long*/ list = GTK.gtk_container_get_children(eventHandle);
...
void setBackgroundRGBA (int /*long*/ handle, GdkRGBA rgba) {
	assert GTK.GTK3 : "GTK3 code was run by GTK2";
...
void setForegroundRGBA (int /*long*/ handle, GdkRGBA rgba) {
	assert GTK.GTK3 : "GTK3 code was run by GTK2";
	if (GTK.GTK_VERSION < OS.VERSION(3, 14, 0)) {
