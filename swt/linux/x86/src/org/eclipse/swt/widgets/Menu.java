...
void createIMMenu (int /*long*/ imHandle) {
	...
	if (imItem == 0) {
		...
		if (GTK.GTK3) {
			...
			int /*long*/ imageHandle = 0;
			int /*long*/ labelHandle = GTK.gtk_accel_label_new (buffer);
			...
			int /*long*/ boxHandle = gtk_box_new (GTK.GTK_ORIENTATION_HORIZONTAL, false, 0);
	...
	int /*long*/ imSubmenu = GTK.gtk_menu_new ();
 ...
 */
void adjustParentWindowWayland (int /*long*/ eventPtr) {
	if (!OS.isX11()) {
		int /*long*/ display = GDK.gdk_display_get_default ();
		int /*long*/ pointer = GDK.gdk_get_pointer(display);
		int /*long*/ deviceWindow = GDK.gdk_device_get_window_at_position(pointer, null, null);
	...
	if (GTK.GTK3 && OS.isX11()) {
		if (itemCount != poppedUpCount && poppedUpCount != 0) {
			...
			if (naturalHeight[0] > 0) {
				int /*long*/ topLevelWidget = GTK.gtk_widget_get_toplevel(handle);
				int /*long*/ topLevelWindow = GTK.gtk_widget_get_window(topLevelWidget);
