...
public class Clipboard {
	...
	static {
		...
		int /*long*/ primary = GDK.gdk_atom_intern(buffer, false);
...
public String[] getAvailableTypeNames() {
	...
	for (int i = 0; i < types1.length; i++) {
		int /*long*/ pName = GDK.gdk_atom_name(types1[i]);
		if (pName == 0) {
			continue;
		}
	...
	for (int i = 0; i < types2.length; i++) {
		int /*long*/ pName = GDK.gdk_atom_name(types2[i]);
		if (pName == 0) {
			continue;
		}
		byte[] buffer = new byte [C.strlen(pName)];
...
int /*long*/ gtk_clipboard_wait_for_contents(int /*long*/ clipboard, int /*long*/ target) {
	...
	int /*long*/ selection_data = GTK.gtk_clipboard_wait_for_contents(clipboard, target);
