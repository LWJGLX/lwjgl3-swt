...
void drag_data_received ( int /*long*/ widget, int /*long*/ context, int x, int y, int /*long*/ selection_data, int info, int time){
	...
	int /*long*/ type = GTK.gtk_selection_data_get_data_type(selection_data);
	if (data != 0) {
...
int getOperationFromKeyState() {
	...
	if (GTK.GTK3) {
		...
		int /*long*/ pointer = GDK.gdk_get_pointer (GDK.gdk_window_get_display (root));
...
public void setTransfer(Transfer... transferAgents){
	...
	if (control instanceof Combo) {
		if ((control.getStyle() & SWT.READ_ONLY) == 0) {
			int /*long*/ entryHandle = GTK.gtk_bin_get_child (control.handle);
			if (entryHandle != 0) {
...
boolean setEventData(int /*long*/ context, int x, int y, int time, DNDEvent event) {
	...
	int /*long*/ window = GTK.gtk_widget_get_window (control.handle);
