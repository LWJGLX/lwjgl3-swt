		...
		if (COM.StgIsStorageFile(fileName) == COM.S_OK) {
			...
			try {
				...
				while((count = fileInput.read(buffer)) > 0){
					int /*long*/ pv = OS.CoTaskMemAlloc(count);
...
private int GetContainer(int /*long*/ ppContainer) {
	 ...
	 */
	if (ppContainer != 0)
		OS.MoveMemory(ppContainer, new int /*long*/[]{0}, C.PTR_SIZEOF);
...
protected int GetWindow(int /*long*/ phwnd) {
	...
	if (frame == null) {
		OS.MoveMemory(phwnd, new int /*long*/[] {0}, C.PTR_SIZEOF);
		return COM.E_NOTIMPL;
	}

	// Copy the Window's handle into the memory passed in
	OS.MoveMemory(phwnd, new int /*long*/[] {handle}, C.PTR_SIZEOF);
	return COM.S_OK;
}
RECT getRect() {
...
private int GetWindowContext(int /*long*/ ppFrame, int /*long*/ ppDoc, int /*long*/ lprcPosRect, int /*long*/ lprcClipRect, int /*long*/ lpFrameInfo) {
	...
	OS.MoveMemory(ppFrame, new int /*long*/[] {iOleInPlaceFrame}, C.PTR_SIZEOF);
	frame.AddRef();

	// null out document handle
	if (ppDoc != 0) OS.MoveMemory(ppDoc, new int /*long*/[] {0}, C.PTR_SIZEOF);
...
protected int QueryInterface(int /*long*/ riid, int /*long*/ ppvObject) {
	...
	if (COM.IsEqualGUID(guid, COM.IIDIAdviseSink)) {
		OS.MoveMemory(ppvObject, new int /*long*/[] {iAdviseSink.getAddress()}, C.PTR_SIZEOF);
		AddRef();
		return COM.S_OK;
	}
	if (COM.IsEqualGUID(guid, COM.IIDIOleClientSite)) {
		OS.MoveMemory(ppvObject, new int /*long*/[] {iOleClientSite.getAddress()}, C.PTR_SIZEOF);
		AddRef();
		return COM.S_OK;
	}
	if (COM.IsEqualGUID(guid, COM.IIDIOleInPlaceSite)) {
		OS.MoveMemory(ppvObject, new int /*long*/[] {iOleInPlaceSite.getAddress()}, C.PTR_SIZEOF);
		AddRef();
		return COM.S_OK;
	}
	if (COM.IsEqualGUID(guid, COM.IIDIOleDocumentSite )) {
		...
		if (!progID.startsWith("PowerPoint")) { //$NON-NLS-1$
			OS.MoveMemory(ppvObject, new int /*long*/[] {iOleDocumentSite.getAddress()}, C.PTR_SIZEOF);
			AddRef();
			return COM.S_OK;
		}
	}
...
private boolean saveFromContents(int /*long*/ address, File file) {
	...
	try {
		...
		int increment = 1024 * 4;
		int /*long*/ pv = OS.CoTaskMemAlloc(increment);
...
private boolean saveFromOle10Native(int /*long*/ address, File file) {
	...
	int /*long*/ pv = OS.CoTaskMemAlloc(4);
	int[] size = new int[1];
