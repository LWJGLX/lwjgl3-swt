...
private int Invoke(int dispIdMember, int /*long*/ riid, int lcid, int dwFlags, int /*long*/ pDispParams, int /*long*/ pVarResult, int /*long*/ pExcepInfo, int /*long*/ pArgErr) {
	if (pVarResult == 0 || dwFlags != COM.DISPATCH_PROPERTYGET) {
		if (pExcepInfo != 0) OS.MoveMemory(pExcepInfo, new int /*long*/ [] {0}, C.PTR_SIZEOF);
	...
	switch (dispIdMember) {
		...
		case COM.DISPID_AMBIENT_SHOWHATCHING :
			if (pVarResult != 0) OS.MoveMemory(pVarResult, new int /*long*/ [] {0}, C.PTR_SIZEOF);
			if (pExcepInfo != 0) OS.MoveMemory(pExcepInfo, new int /*long*/ [] {0}, C.PTR_SIZEOF);
			if (pArgErr != 0) OS.MoveMemory(pArgErr, new int[] {0}, 4);
			return COM.S_FALSE;
...
protected int QueryInterface(int /*long*/ riid, int /*long*/ ppvObject) {
	...
	if (COM.IsEqualGUID(guid, COM.IIDIOleControlSite)) {
		OS.MoveMemory(ppvObject, new int /*long*/[] {iOleControlSite.getAddress()}, C.PTR_SIZEOF);
		AddRef();
		return COM.S_OK;
	}
	if (COM.IsEqualGUID(guid, COM.IIDIDispatch)) {
		OS.MoveMemory(ppvObject, new int /*long*/[] {iDispatch.getAddress()}, C.PTR_SIZEOF);
		AddRef();
		return COM.S_OK;
	}
