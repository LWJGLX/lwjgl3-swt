...
void getData(int /*long*/ pData){
	...
	if ((type & COM.VT_BYREF) == COM.VT_BYREF) {
		...
		OS.MoveMemory(pData + 8, new int /*long*/[]{byRefPtr}, C.PTR_SIZEOF);
		return;
	}
	...
	switch (type) {
		...
		case COM.VT_DISPATCH :
			dispatchData.AddRef();
			OS.MoveMemory(pData, new short[] {type}, 2);
			OS.MoveMemory(pData + 8, new int /*long*/[]{dispatchData.getAddress()}, C.PTR_SIZEOF);
			break;
		case COM.VT_UNKNOWN :
			unknownData.AddRef();
			OS.MoveMemory(pData, new short[] {type}, 2);
			OS.MoveMemory(pData + 8, new int /*long*/[]{unknownData.getAddress()}, C.PTR_SIZEOF);
			break;
		case COM.VT_BSTR :
			OS.MoveMemory(pData, new short[] {type}, 2);
			char[] data = (stringData+"\0").toCharArray();
			int /*long*/ ptr = COM.SysAllocString(data);
			OS.MoveMemory(pData + 8, new int /*long*/[] {ptr}, C.PTR_SIZEOF);
			break;
...
public void setByRef(int /*long*/ val) {
	...
	OS.MoveMemory(byRefPtr, new int /*long*/[]{val}, C.PTR_SIZEOF);
}
/**
