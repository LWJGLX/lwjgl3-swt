...
private int EnumFormatEtc(int dwDirection, int /*long*/ ppenumFormatetc) {
	...
	OS.MoveMemory(ppenumFormatetc, new int /*long*/[] {enumFORMATETC.getAddress()}, C.PTR_SIZEOF);
	return COM.S_OK;
}
/**
...
private int QueryInterface(int /*long*/ riid, int /*long*/ ppvObject) {
	...
	if (COM.IsEqualGUID(guid, COM.IIDIUnknown) || COM.IsEqualGUID(guid, COM.IIDIDropSource)) {
		OS.MoveMemory(ppvObject, new int /*long*/[] {iDropSource.getAddress()}, C.PTR_SIZEOF);
		AddRef();
		return COM.S_OK;
	}

	if (COM.IsEqualGUID(guid, COM.IIDIDataObject) ) {
		OS.MoveMemory(ppvObject, new int /*long*/[] {iDataObject.getAddress()}, C.PTR_SIZEOF);
		AddRef();
		return COM.S_OK;
	}
