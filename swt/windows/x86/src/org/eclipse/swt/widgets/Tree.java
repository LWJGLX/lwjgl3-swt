...
int /*long*/ getNextSelection (int /*long*/ hItem) {
	while (hItem != 0) {
		...
		int /*long*/ hFirstItem = OS.SendMessage (handle, OS.TVM_GETNEXTITEM, OS.TVGN_CHILD, hItem);
		int /*long*/ hSelected = getNextSelection (hFirstItem);
		...
		case ' ': {
			...
			if (hItem != 0) {
				...
				if ((style & SWT.CHECK) != 0) {
					...
					OS.SendMessage (handle, OS.TVM_SETITEM, 0, tvItem);
					int /*long*/ id = OS.SendMessage (handle, OS.TVM_MAPHTREEITEMTOACCID, hItem, 0);
	...
	if (lpht.hItem != 0) {
		if ((style & SWT.CHECK) != 0) {
			if ((lpht.flags & OS.TVHT_ONITEMSTATEICON) != 0) {
				...
				tvItem.state = state << 12;
				OS.SendMessage (handle, OS.TVM_SETITEM, 0, tvItem);
				int /*long*/ id = OS.SendMessage (handle, OS.TVM_MAPHTREEITEMTOACCID, tvItem.hItem, 0);
				OS.NotifyWinEvent (OS.EVENT_OBJECT_FOCUS, handle, OS.OBJID_CLIENT, (int)/*64*/id);
				Event event = new Event ();
		...
		case OS.NM_CUSTOMDRAW: {
			...
			switch (nmcd.dwDrawStage) {
				...
				case OS.CDDS_ITEMPREPAINT: {
					...
					int /*long*/ brush = OS.CreateSolidBrush(pixel);
				...
				case OS.CDDS_POSTPAINT: {
					...
					for (int i=0; i<columnCount; i++) {
						...
						if (columns[i] == sortColumn && sortDirection != SWT.NONE) {
							...
							int /*long*/ pen = OS.CreatePen (OS.PS_SOLID, 1, getHeaderForegroundPixel());
							int /*long*/ oldPen = OS.SelectObject (nmcd.hdc, pen);
						...
						int /*long*/ pen = OS.CreatePen (OS.PS_SOLID, getGridLineWidthInPixels(), getSlightlyDifferentColor(getHeaderBackgroundPixel()));
						int /*long*/ oldPen = OS.SelectObject (nmcd.hdc, pen);
					...
					if (lastColumnRight < nmcd.right) {
						...
						int /*long*/ brush = OS.CreateSolidBrush(getHeaderBackgroundPixel());
						OS.FillRect(nmcd.hdc, rect, brush);
						OS.DeleteObject(brush);
					}
					...
					if (highlightedHeaderDividerX != -1) {
						int /*long*/ pen = OS.CreatePen (OS.PS_SOLID, 4, OS.GetSysColor(OS.COLOR_HIGHLIGHT));
						int /*long*/ oldPen = OS.SelectObject (nmcd.hdc, pen);
