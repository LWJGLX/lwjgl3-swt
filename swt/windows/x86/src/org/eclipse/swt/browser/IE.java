		...
		if (auto != null) {
			switch (event.type) {
				case BeforeNavigate2: {
					...
					if (uncRedirect != null) {
						...
						*/
						if (uncRedirect.equals(url1) || (uncRedirect.startsWith(url1) && uncRedirect.indexOf('\\', 2) == url1.length())) {
							...
							if (cancel1 != null) {
								int /*long*/ pCancel1 = cancel1.getByRef();
					...
					*/
					...
					if (url1.startsWith(PROTOCOL_FILE) && _getUrl().startsWith(ABOUT_BLANK) && untrustedText) {
						...
						if (cancel2 != null) {
							int /*long*/ pCancel2 = cancel2.getByRef();
					...
					if (cancel3 != null) {
						int /*long*/ pCancel3 = cancel3.getByRef();
					...
					if (url4.startsWith("\\\\")) { //$NON-NLS-1$
						...
						if (statusCode == INET_E_RESOURCE_NOT_FOUND) {
							...
							if (index != -1) {
								...
								if (cancel4 != null) {
									int /*long*/ pCancel4 = cancel4.getByRef();
				...
				case NewWindow2: {
					...
					int /*long*/ pCancel5 = cancel5.getByRef();
					...
					if (doit2) {
						...
						*/
						...
						if (byref != 0) OS.MoveMemory(byref, new int /*long*/[] {iDispatch.getAddress()}, C.PTR_SIZEOF);
					}
				...
				case WindowClosing: {
					...
					*/
					...
					int /*long*/ pCancel6 = cancel6.getByRef();
